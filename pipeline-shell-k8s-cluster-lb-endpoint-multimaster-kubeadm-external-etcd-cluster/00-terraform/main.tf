...
# ----------------------------------------------------------------------------
# Master node ETCD instance template

resource "aws_instance" "k8s_master_etcd" {
  count = var.instance_k8s_master_etcd_object.number_of_nodes
  # ami                         = data.aws_ami.ubuntu.image_id
  ami           = "ami-09c92d3eb1db3a728"
  instance_type = var.instance_k8s_master_etcd_object.type
  key_name      = var.instance_k8s_master_etcd_object.key_pair
  # iam_instance_profile        = var.instance_k8s_master_etcd_object.iam_profile
  associate_public_ip_address = var.instance_k8s_master_etcd_object.public_ip
  ...
  root_block_device {
    delete_on_termination = true
    encrypted             = true
    volume_size           = var.instance_k8s_master_etcd_object.root_ebs_size
  }
