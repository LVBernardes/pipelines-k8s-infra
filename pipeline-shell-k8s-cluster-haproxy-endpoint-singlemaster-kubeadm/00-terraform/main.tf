...
# ----------------------------------------------------------------------------
# HAProxy instance template

resource "aws_instance" "haproxy" {
  count = var.instance_hapŕoxy_object.number_of_nodes
  # ami                         = data.aws_ami.ubuntu.image_id
  ami           = "ami-09c92d3eb1db3a728"
  instance_type = var.instance_hapŕoxy_object.type
  key_name      = var.instance_hapŕoxy_object.key_pair
  # iam_instance_profile        = var.instance_k8s_master_object.iam_profile
  associate_public_ip_address = var.instance_hapŕoxy_object.public_ip
  ...
  root_block_device {
    delete_on_termination = true
    encrypted             = true
    volume_size           = var.instance_hapŕoxy_object.root_ebs_size
  }
  ...
  tags = {
    "Name"                      = "${var.instance_hapŕoxy_object.tags.name}-${count.index}"
    "instance-group"            = "${var.instance_hapŕoxy_object.tags.instance_group}"
    "instance-sub-group"        = "${var.instance_hapŕoxy_object.tags.instance_sub_group}"
    "ctr-orchestration-engine"  = "${var.container_orchestration.engine}"
    "ctr-orchestration-cluster" = "${var.container_orchestration.cluster_name}"
    "ctr-orchestration-network" = "${var.container_orchestration.network_template}"
    "ctr-engine"                = "${var.container_engine.name}"
